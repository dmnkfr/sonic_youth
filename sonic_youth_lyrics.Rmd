...
# Some helpers
records_by_year = c("Confusion Is Sex", "Bad Moon Rising", "EVOL",
...
ggplot(data = words_per_song, aes(x = reorder(album, number_unique_words_per_song), y = number_unique_words_per_song, label = song)) +
...
lyrics_tf_idf %>%
    mutate(word = reorder_within(word, tf_idf, album),
           album = factor(album, levels=records_by_year)) %>%
...
```{r topic-model-setup, message=FALSE, results="hide"}
