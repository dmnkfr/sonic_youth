...
frequent_words %>%
  ...
  geom_text(aes(label=ifelse(frequency > 200,paste(as.character(frequency), " times"),"")),
    ...
    geom_text(aes(label=ifelse(docfreq > 60,paste("in ", as.character(docfreq), " songs"),"")),
            hjust=-0.4,vjust=0, size = 3) +
...
[![](images/sy.jpg "Frans Schellekens/Redferns")](https://www.rollingstone.com/music/music-news/hot-band-sonic-youth-45573/)
...
```{r lyrics-classifier1, fig.dim = c(7,4), echo=FALSE}
# load model
...
```{r lyrics-classifier2, fig.dim = c(7,4), echo=FALSE}
# load model
model_class = load_model_tf("model")
# define input
input = "I'll kiss your shadow in the sand"
