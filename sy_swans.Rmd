...
# add album info
all_ids = data.frame(song_id = unique(sw_lyrics$song_id))
all_ids$album = ""
...
**Fantastic!** We got 4602 lines of poetry from 149 Sonic Youth song and 4407 lines of prayers from 146 Swans songs. Let's see how many songs are on each of the records.

```{r combine-lyrics, eval=FALSE}
...
```{r print-tables, message=FALSE, results="show"}
...
kable(list(t1, t2), caption = c("Swans", "Sonic Youth"))
```

<br/> Let's explore the words in this noise
rock landscape!

### How many words do they use on each album?

```{r plot-words-per-records, message=FALSE, warning=FALSE, results = "hide", fig.dim = c(14,5)}
lyrics = readRDS("./interim/lyrics_tidy.RDS")
...
### How long are their lyrics per song?

A quick and dirty count of unique words per song. We exclude all the la la la's and uh uh uh's.

```{r plot-words-per-song, message=FALSE, results = "hide", fig.dim = c(13,5)}
...
```{r process-tokens, eval=FALSE, warning=FALSE}
...
sw_tokens = tokens_ngrams(sw_tokens, n = 1:3)
...
# Add bigrams and trigrams to our feature matrix
sy_tokens = tokens_ngrams(sy_tokens, n = 1:3)
...
```{r wordclouds, results="hide", fig.dim=c(13,8)}
...
```{r frequent_words, message=FALSE, results="show", fig.dim=c(7,5)}
# frequent words
...
frequent_words %>%
  ...
  geom_text(aes(label=ifelse(frequency > 200,paste(as.character(frequency), " times"),"")),
    ...
    geom_text(aes(label=ifelse(artist == "Swans" & word == "love",paste("on ", as.character(docfreq), " songs"),"")),
